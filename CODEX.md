...
Проверить target, существующий extent, новый zvol/clone и принадлежность станции. Не менять имя target, association extent и букву `D:`. В существующем extent заменить только `device/file` на путь нового zvol через `iscsi.extent.update`. Переключать выбранные станции в worker с прогрессом по каждой станции.
...
Адаптер должен уметь читать zvol/dataset/snapshot/clone, iSCSI target/extent/association и выполнять только явно разрешённые операции snapshot, clone и замену `device/file` существующего extent. Target association и LUN не переключать. Все методы/параметры сверять с онлайн-документами и фактической `/api/docs/` при интеграционном тесте.
