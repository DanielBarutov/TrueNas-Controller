...
При настроенном read-only подключении TrueNAS экран по умолчанию показывает
только физически существующие tracked dataset. Старые строки Controller,
которых уже нет в TrueNAS, доступны отдельно через «Показывать удалённые».
